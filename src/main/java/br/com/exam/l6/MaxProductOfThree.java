...
public class MaxProductOfThree {
	...
	public static void main(String[] args) {
		...
		System.out.println(solution(new int[] { -5, 5, -5, 4 }));//125
		System.out.println(solution(new int[] { -5, -6, -4, -7, -10 }));//-120
	}

	public static int solution(int[] A) {
		java.util.Arrays.sort(A);
		if(A[A.length - 1] > 0 && (A[0] * A[1] > A[A.length - 1] * A[A.length - 2])){
			return A[0] * A[1] * A[A.length - 1];
		}

		if(A[A.length - 1] > 0 && A[0] * A[1] > A[A.length - 2] * A[A.length - 3]){
			return A[0] * A[1] * A[A.length - 1];
		}
